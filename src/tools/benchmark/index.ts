...
export async function executeBenchmarkTool(name: string, args: Record<string, unknown>) {
  ...
  switch (name) {
    case "benchmark_function":
      return await handler.benchmarkFunction(args as { code: string; iterations?: number; warmup?: number });
    case "benchmark_comparison":
      return await handler.benchmarkComparison(args as { functions: Array<{ name: string; code: string }>; iterations?: number });
    case "benchmark_http_endpoint":
      return await handler.benchmarkHttpEndpoint(args as { url: string; method?: string; concurrent?: number; duration?: number });
    ...
    case "benchmark_sorting_algorithms":
      return await handler.benchmarkSortingAlgorithms(args as { size?: number; algorithms?: string[] });
    case "benchmark_memory_usage":
      return await handler.benchmarkMemoryUsage(args as { code: string; iterations?: number });
    default:
